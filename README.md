...
Materials may also provide a WGSL implementation. The optional Node backend uses the `webgpu` Dawn binding, renders offscreen at terminal resolution, and overlaps GPU work with reusable readback rings. Select `auto`, `cpu`, or `gpu` from the global home menu or a game menu; a missing package or adapter leaves the CPU renderer active. The GPU path covers the prism and splash, Cover Flow and its launch flip, chess, Poker and the card showcase (including textured cards and chips), Catan (including animated dice), and procedural wisps. TUI layout, projected labels, terminal-cell compositing, and ANSI output remain on the CPU because they are terminal presentation rather than framebuffer geometry. No browser or rendering service is involved. `pnpm bench:gpu 140 50 30` compares Catan CPU rendering against GPU submission plus readback; `pnpm smoke:gpu 140 50` validates representative GPU frames across every visual mode.

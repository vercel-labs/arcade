...
| 4 | **Chess** | chess | click a white piece — legal squares light up (`chess.select`) · click a lit square to move (`chess.move` / `chess.capture`) · `h` or the *moves* pill hides/shows the history (`chess.history`) · *new match* opens the model picker (`chess.setup`) · pick a side and a model, start — a real match billed through AI Gateway, using monthly included credit for free-tier models or purchased Gateway Credits for other models, with the normal health-check popups (`chess.matchStarted`, gateway) · click a wisp to swap its model (`chess.swap`, gateway) |

# CATAN — rules, phases, and implementation design

The research + design reference for adding **Catan** (base game, 3–4 players) to the
...
rules below follow the official *CATAN Game Rules & Almanac* (5th English edition, ©
2015/2020 Catan GmbH & Catan Studio), which supersedes all earlier printings; digital-flow
...
How well-made digital clients (primarily **colonist.io**, cross-checked against Catan
...
- **Prune stale state:** rejected/unaffordable offers vanish so the visible set equals the
  actionable set.
- Base Catan bounds this: only the active player initiates; no 3-way/secret/credit trades;
...
How Catan fits the arcade's OpenSpiel-style harness ([`src/rules/game.ts`](../src/rules/game.ts)),
informed by a comparative study of open-source engines (catanatron is the reference).

## 3.1 The harness contract

- One **`CatanState` = one full game** (to 10 VP). Unlike poker (where a `HoldemState` is
  one hand and the session lives in the driver), a Catan game is a single state's lifetime —
  closer to `ChessState`.
- It implements **`ImperfectInfoState<CatanAction>`**: dev cards, the dev-deck order, and
...
(catanatron notwithstanding, it also resolves internally) or a search-first design would
prefer.
...
- **Dedup by canonical geometric key** (no floats): every vertex is the meeting of a
  *triple* of hex positions (self + two angularly-adjacent neighbors, some possibly
  ...
  — the integer-ID version of catanatron's construction-time dedup, without hand-listing
  arrays.
...
This is [`board-topology.ts`](../src/rules/catan/board-topology.ts) — pure, deterministic,
...
- **Hidden:** each player's exact resource identities, each player's dev cards, the ordered
  ...
  later — an idea worth stealing from `settlers_of_catan_RL`).
...
- **Domestic trade is gated behind a flag, off by default for AI.** Every source agrees
  trading is the hard part; catanatron's RL space omits it entirely and RL agents flail with
...
- **Bot ladder** (recommended order, per catanatron's benchmarks): (1) random (respecting
  ...
  effort (catanatron found 3-ply *worse* than 2-ply; value function > deeper search); (4)
...
Linear: team **AI Gateway**, project **Arcade**, **Catan** milestone. AIG-205 (registry
...
- [Official CATAN Base Game Rules & Almanac (5th ed., 2020 PDF), catan.com](https://www.catan.com/sites/default/files/2021-06/catan_base_rules_2020_200707.pdf) — primary source for components, setup, initial placement, turn structure, costs, dev cards, robber, trading, special cards, victory, and almanac edge-case rulings.
- [CATAN Game Rules hub, catan.com](https://www.catan.com/understand-catan/game-rules) — official index confirming the current base edition.
- [Official CATAN Base Game FAQ/Almanac, catan.com](https://www.catan.com/faq/basegame) — definitive edge-case rulings (dev-card-plus-build, knight-before-roll, no-card-same-turn-bought, win-only-on-your-turn, Largest Army/Longest Road transfer & ties, robber-must-move/desert-allowed, empty-hand steal, discard rounding, empty-bank production, Monopoly honesty, trading restrictions).
- [Wikipedia: Catan](https://en.wikipedia.org/wiki/Catan) — cross-check for terrain→resource, production, robber, trade ratios, dev cards, 10-VP victory.
- [CatanBoard setup guide](https://catanboard.com/setup) — cross-check for per-type hex distribution and harbor split (4× 3:1 + 5× 2:1, per the official components list).

### Digital flow (colonist.io + others)

- [colonist.io/catan-rules](https://colonist.io/catan-rules) — colonist's base-game rules: turn order, production, 7/robber/discard, trade rates, costs/limits, dev-card restrictions, special cards, snake setup, second-settlement resources.
...
- [Guide to Catan starting strategies (colonist)](https://blog.colonist.io/guide-to-catan-starting-strategies/) — snake-order detail, highlighted valid placements, probability "dots", second-settlement seeding.
- [Board Game Arena: Catan help](https://en.doc.boardgamearena.com/Gamehelpcatan) — corroborating turn structure, robber-to-new-terrain, trade types, dev-card timing, tie-keeps-owner, win-only-on-your-turn.

### Implementations & AI

- [catanatron (GitHub)](https://github.com/bcollazo/catanatron) and [docs](https://catanatron.readthedocs.io/en/latest/catanatron.models.html) — the reference Python engine: cube-coord hexes + integer node/edge IDs, construction-time dedup, `ActionPrompt` state machine, `current_player_index` vs `current_turn_index`, freqdeck bank + ordered dev deck, longest-road caches, bot ladder.
- [catanatron gym action_space.py](https://raw.githubusercontent.com/bcollazo/catanatron/master/catanatron/catanatron/gym/envs/action_space.py) — flattened action enumeration and per-type counts (328 for 1v1 base); confirms domestic trade excluded from the RL space.
- ["5 Ways NOT to Build a Catan AI" (bcollazo, Medium)](https://medium.com/@bcollazo2010/5-ways-not-to-build-a-catan-ai-e01bc491af17) — AlphaBeta n=2 strongest; 3-ply worse than 2-ply; value function > deeper search.
- [settlers-rl.github.io](https://settlers-rl.github.io/) and [settlers_of_catan_RL (GitHub)](https://github.com/henrycharlesworth/settlers_of_catan_RL) — deep-RL design: attention encoder over tiles, min/max hidden-info belief encoding, factored action heads + masking, recurrent trade heads, "trading never mastered."
- [m-l.dev: building a Catan board with NetworkX](https://m-l.dev/catan-board/) — graph board via `hexagonal_lattice_graph` + `minimum_cycle_basis`; dedup-for-free; "push adjacency complexity to construction."
- [Szita, Chaslot & Spronck 2009, "MCTS in Settlers of Catan"](https://link.springer.com/chapter/10.1007/978-3-642-12993-3_3) — canonical high-branching + stochastic benchmark; determinized MCTS.
...
- [OriGoldfrydCS/Settlers-of-Catan (C++)](https://github.com/OriGoldfrydCS/Settlers-of-Catan) — 1–54 numbered intersections, clean `canPlace*/place*` rules API.
- [nbelle1/strategy-game-agents](https://github.com/nbelle1/strategy-game-agents) — a fork adding an `LLM` player to catanatron (compact text obs + validated actions).

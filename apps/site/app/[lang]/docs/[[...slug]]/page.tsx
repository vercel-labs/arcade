...
export default async function DocsPage({ params }: PageProps<'/[lang]/docs/[[...slug]]'>) {
  ...
  const nestedMode = page.slug === 'reference' || page.navParent === 'reference' ? 'reference' : page.slug === 'guides' || page.navParent === 'guides' ? 'guides' : page.slug === 'games' || page.navParent === 'games' ? 'games' : null;
  const navItems = nestedMode === 'reference' ? nestedNavItems(REFERENCE_DOCS, 'reference') : nestedMode === 'guides' ? nestedNavItems(GUIDE_DOCS, 'guides') : nestedMode === 'games' ? nestedNavItems(GAME_DOCS, 'games') : CORE_NAV;

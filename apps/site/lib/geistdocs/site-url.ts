...
const resolveSiteUrl = () => {
  if (!configuredUrl) {
    return { issue: 'NEXT_PUBLIC_VERCEL_PROJECT_PRODUCTION_URL is not set', url: fallbackSiteUrl };
  }
  const value =
  ...
  try {
    ...
    return { issue: 'NEXT_PUBLIC_VERCEL_PROJECT_PRODUCTION_URL is invalid', url: fallbackSiteUrl };
  }
};

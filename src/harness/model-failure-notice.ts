...
// rather than only naming the symptom. This pauses (persistent) rather than playing
// a random legal move: the AI SDK already retried with backoff before surfacing it,
// and a silent random move would misreport how the model actually plays.
const RATE_LIMIT_BODY = (model: string): string =>
  `${model} hit AI Gateway's rate limit. free-tier teams get a lower per-model limit. adding AI Gateway credit (even $10) moves your team to the paid tier, which removes it. retry to keep playing.`;
...
export function modelFailureNotice(failure: ClassifiedError, model: string): ModelFailureNotice | null {
  ...
  switch (failure.gatewayType) {
    ...
    case 'rate_limit_exceeded': return { code: 'rate_limit_exceeded', severity: 'caution', title: 'rate limit reached', body: RATE_LIMIT_BODY(model), persistent: true, action: { label: 'view AI Gateway logs', url: GATEWAY_LOGS_URL } };
  ...
  if (failure.status === 429) return { code: 'rate_limit', severity: 'caution', title: 'rate limit reached', body: RATE_LIMIT_BODY(model), persistent: true, action: { label: 'view AI Gateway logs', url: GATEWAY_LOGS_URL } };

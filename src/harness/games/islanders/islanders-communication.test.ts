...
import { IslandersCommunicationCoordinator, islandersActionSalience } from './islanders-communication.ts';

test('Islanders action salience distinguishes negotiations from routine turn mechanics', () => {
  assert.ok(islandersActionSalience({ type: 'offerTrade', give: [1, 0, 0, 0, 0], receive: [0, 1, 0, 0, 0] }) > islandersActionSalience({ type: 'roll' }));
  assert.ok(islandersActionSalience({ type: 'playMonopoly', resource: 'ore' }) > islandersActionSalience({ type: 'endTurn' }));
});

test('only ambient mode teaches models the public seat map for directed speech', () => {
  const ambient = new IslandersCommunicationCoordinator('ambient', ['A', 'B']).modelConfig().guide;
  const autoreply = new IslandersCommunicationCoordinator('autoreply', ['A', 'B']).modelConfig().guide;
...
test('human table talk is public context and a direct target must be admitted by ambient policy', () => {
  const coordinator = new IslandersCommunicationCoordinator('ambient', ['the human player', 'Claude']);
...
test('ambient suppresses routine captions while autoreply preserves the compatibility behavior', () => {
  const ambient = new IslandersCommunicationCoordinator('ambient', ['A', 'B']);
...
test('ambient admits an encouraged high-importance reaction opportunity', () => {
  const coordinator = new IslandersCommunicationCoordinator('ambient', ['A', 'B']);
  const decision = coordinator.decideOpportunity({
    seat: 1,
    expectation: 'encouraged',
    reason: 'directly affected by the moment',
    moment: {
      id: 'islanders-12-1', game: 'islanders', type: 'route_cutoff', actorSeat: 0,
...
test('directed model dialogue gets one reply beat and cannot recurse', () => {
  const coordinator = new IslandersCommunicationCoordinator('ambient', ['A', 'B', 'C']);
  ...
  const [opportunity] = directedReplyOpportunities(message!, 'islanders', 3);

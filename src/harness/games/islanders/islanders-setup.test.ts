import assert from 'node:assert/strict';
import { test } from 'node:test';
import { MockLanguageModelV3 } from 'ai/test';
import type { GameState } from '../../../rules/game.ts';
import { IslandersState } from '../../../rules/islanders/islanders.ts';
import type { IslandersAction } from '../../../rules/islanders/types.ts';
import type { Player } from '../../player.ts';
import {
  type IslandersSetupScene,
  IslandersMatchActionLimitError,
  createIslandersModelPlayer,
  runIslandersMatch,
  createIslandersSetupModelPlayer,
  runIslandersInitialPlacement,
} from './islanders-setup.ts';

function rng(seed = 0xc47a): () => number {
  let a = seed >>> 0;
  return () => {
    a |= 0;
    a = (a + 0x6d2b79f5) | 0;
    let x = Math.imul(a ^ (a >>> 15), 1 | a);
    x = (x + Math.imul(x ^ (x >>> 7), 61 | x)) ^ x;
    return ((x ^ (x >>> 14)) >>> 0) / 4294967296;
  };
}

class SetupScene implements IslandersSetupScene {
  readonly actions: IslandersAction[] = [];

  constructor(private readonly game: IslandersState) {}

  state(): IslandersState {
    return this.game;
  }

  async playMove(action: IslandersAction): Promise<void> {
    this.actions.push(action);
    this.game.applyAction(action);
  }
}

class GreedySetupPlayer implements Player<IslandersAction> {
  calls = 0;

  constructor(readonly name: string) {}

  async chooseAction(state: GameState<IslandersAction>): Promise<{ action: IslandersAction; rationale: string }> {
    this.calls++;
    const islanders = state as IslandersState;
    if (islanders.currentPrompt().kind === 'initialSettlement') {
      const option = islanders
        .initialSettlementOptions()
        .reduce((best, candidate) =>
          candidate.totalPips + candidate.resourceDiversity > best.totalPips + best.resourceDiversity ? candidate : best,
        );
      return { action: option.action, rationale: `N${option.node}: ${option.totalPips} pips` };
    }
    const score = (option: ReturnType<IslandersState['initialRoadOptions']>[number]): number =>
      Math.max(0, ...option.expansionSites.map((site) => site.totalPips));
    const option = islanders.initialRoadOptions().reduce((best, candidate) => (score(candidate) > score(best) ? candidate : best));
    return { action: option.action, rationale: `E${option.edge}: expands toward N${option.towardNode}` };
  }
}

test('setup runner drives four players through exactly the initial snake and stops before roll', async () => {
  const state = new IslandersState({ numPlayers: 4, rng: rng(), seatNames: ['A', 'B', 'C', 'D'] });
  const scene = new SetupScene(state);
  const players = ['A', 'B', 'C', 'D'].map((name) => new GreedySetupPlayer(name));
  const thinking: number[] = [];
  const commentary: string[] = [];

  const result = await runIslandersInitialPlacement(scene, players, {
    onThinking: (_player, seat) => thinking.push(seat),
    onCommentary: (text) => commentary.push(text),
  });

  assert.equal(result, state);
  assert.equal(state.initialPlacementComplete(), true);
  assert.equal(state.currentPrompt().kind, 'roll');
  assert.equal(scene.actions.length, 16);
  assert.deepEqual(players.map((player) => player.calls), [4, 4, 4, 4]);
  assert.deepEqual(
    thinking.filter((_, index) => scene.actions[index].type === 'initialSettlement'),
    [0, 1, 2, 3, 3, 2, 1, 0],
  );
  assert.equal(commentary.length, 16);
});

test('setup runner requires exactly one player per Islanders seat', async () => {
  const scene = new SetupScene(new IslandersState({ numPlayers: 4, rng: rng() }));
  await assert.rejects(() => runIslandersInitialPlacement(scene, [new GreedySetupPlayer('only one')]), /one player per seat/);
});

test('full match runner uses the same scene seam beyond setup with no UI dependency', async () => {
  const state = new IslandersState({ numPlayers: 4, rng: rng() });
  const scene = new SetupScene(state);
  const players: Player<IslandersAction>[] = Array.from({ length: 4 }, (_, seat) => ({
    name: `P${seat}`,
    chooseAction: async (game) => ({ action: game.legalActions()[0], rationale: 'first legal action' }),
  }));
  const result = await runIslandersMatch(scene, players, {
    shouldStop: () => state.initialPlacementComplete() && state.currentPlayer() === 1,
  });
  assert.equal(result.state, state);
  assert.equal(result.status, 'bounded');
  assert.equal(result.stopReason, 'stopped');
  assert.equal(result.actionCount, scene.actions.length);
  assert.ok(scene.actions.length >= 18, '16 setup actions, then at least roll and end turn');
  assert.equal(scene.actions[16].type, 'roll');
  assert.equal(scene.actions.at(-1)?.type, 'endTurn');
  assert.deepEqual(state.currentPrompt(), { kind: 'roll', player: 1 });
});

test('full match runner reports a legal but non-progressing evaluation at its action limit', async () => {
  const state = new IslandersState({ numPlayers: 4, rng: rng() });
  const scene = new SetupScene(state);
  const players: Player<IslandersAction>[] = Array.from({ length: 4 }, (_, seat) => ({
    name: `P${seat}`,
    chooseAction: async (game) => ({ action: game.legalActions()[0], rationale: 'first legal action' }),
  }));
  const result = await runIslandersMatch(scene, players, { maxActions: 20 });
  assert.equal(result.state, state);
  assert.equal(result.status, 'bounded');
  assert.equal(result.stopReason, 'action_limit');
  assert.equal(result.actionCount, 20);
  assert.equal(scene.actions.length, 20);
  assert.equal(state.isTerminal(), false);
});

test('the generic ModelPlayer can select a typed legal Islanders setup action', async () => {
  const state = new IslandersState({ numPlayers: 4, rng: rng() });
  const choice = state.initialSettlementOptions().reduce((best, option) => (option.totalPips > best.totalPips ? option : best));
  let request = '';
  const model = new MockLanguageModelV3({
    doGenerate: async (options) => {
      request = JSON.stringify(options.prompt);
      return ({
        content: [
          {
            type: 'text',
            text: JSON.stringify({
              thinking: 'This is the strongest production choice.',
              move: `init-settlement ${choice.node}`,
              say: 'I like this mix of numbers.',
            }),
          },
        ],
        finishReason: { unified: 'stop', raw: undefined },
        usage: { inputTokens: { total: 1 }, outputTokens: { total: 1 } },
        warnings: [],
      }) as unknown as Awaited<ReturnType<MockLanguageModelV3['doGenerate']>>;
    },
  });
  const player = createIslandersSetupModelPlayer({
    model,
    name: 'islanders-model',
  });

  const picked = await player.chooseAction(state);
  assert.match(request, /Legal actions/);
  assert.match(request, new RegExp(`init-settlement ${choice.node} \\[public spot:`));
  assert.match(request, /Facts are descriptive, not recommendations/);
  assert.match(request, /gives one useful public reason/);
  assert.match(request, /Never use raw node, edge, or hex IDs/);
  assert.deepEqual(picked.action, choice.action);
  assert.equal(picked.rationale, 'I like this mix of numbers.', 'only the public say field is surfaced');
  state.applyAction(picked.action);
  assert.equal(state.currentPrompt().kind, 'initialRoad');
});

test('Islanders setup model fallback is deterministic unless a seeded RNG is supplied', async () => {
  const stateA = new IslandersState({ numPlayers: 4, rng: rng(123) });
  const stateB = new IslandersState({ numPlayers: 4, rng: rng(123) });
  const invalidModel = () =>
    new MockLanguageModelV3({
      doGenerate: async () =>
        ({
          content: [{ type: 'text', text: JSON.stringify({ thinking: 'No valid choice.', move: 'not-a-node', say: 'I am still deciding.' }) }],
          finishReason: { unified: 'stop', raw: undefined },
          usage: { inputTokens: { total: 1 }, outputTokens: { total: 1 } },
          warnings: [],
        }) as unknown as Awaited<ReturnType<MockLanguageModelV3['doGenerate']>>,
    });
  const choiceA = await createIslandersSetupModelPlayer({ model: invalidModel(), maxRetries: 0 }).chooseAction(stateA);
  const choiceB = await createIslandersSetupModelPlayer({ model: invalidModel(), maxRetries: 0 }).chooseAction(stateB);
  assert.deepEqual(choiceA.action, stateA.legalActions()[0]);
  assert.deepEqual(choiceB.action, choiceA.action);
  assert.equal(choiceA.diagnostics?.resolution, 'random-fallback');
});

test('full-game ModelPlayer fallback can discover a parameterized domestic-trade example', async () => {
  const state = new IslandersState({ numPlayers: 4, rng: rng(), domesticTrade: true });
  while (!state.initialPlacementComplete()) state.applyAction(state.legalActions()[0]);
  state.applyAction({ type: 'roll' }, { dice: [1, 1] });
  const model = new MockLanguageModelV3({
    doGenerate: async () =>
      ({
        content: [{ type: 'text', text: JSON.stringify({ thinking: 'No valid choice.', move: 'invalid', say: 'I am still deciding.' }) }],
        finishReason: { unified: 'stop', raw: undefined },
        usage: { inputTokens: { total: 1 }, outputTokens: { total: 1 } },
        warnings: [],
      }) as unknown as Awaited<ReturnType<MockLanguageModelV3['doGenerate']>>,
  });
  const player = createIslandersModelPlayer({ model, maxRetries: 0, fallbackRng: () => 0.999 });
  const choice = await player.chooseAction(state);
  assert.equal(choice.action.type, 'offerTrade');
  assert.equal(state.isLegalAction(choice.action), true);
});

test('the full-game model harness exposes and parses responder counteroffers', async () => {
  const state = new IslandersState({ numPlayers: 2, rng: rng(), domesticTrade: true });
  while (!state.initialPlacementComplete()) state.applyAction(state.legalActions()[0]);
  state.applyAction({ type: 'roll' }, { dice: [1, 1] });
  const hands = (state as unknown as { hands: number[][] }).hands;
  hands[0] = [2, 0, 0, 0, 0];
  hands[1] = [0, 2, 0, 0, 0];
  state.applyAction({ type: 'offerTrade', give: [1, 0, 0, 0, 0], receive: [0, 1, 0, 0, 0] });
  let request = '';
  const model = new MockLanguageModelV3({
    doGenerate: async (options) => {
      request = JSON.stringify(options.prompt);
      return ({
        content: [{
          type: 'text',
          text: JSON.stringify({
            thinking: 'Two wheat is worth the brick here.',
            move: 'counter 0/2/0/0/0 for 1/0/0/0/0',
            say: 'I will give two wheat for that brick.',
          }),
        }],
        finishReason: { unified: 'stop', raw: undefined },
        usage: { inputTokens: { total: 1 }, outputTokens: { total: 1 } },
        warnings: [],
      }) as unknown as Awaited<ReturnType<MockLanguageModelV3['doGenerate']>>;
    },
  });
  const choice = await createIslandersModelPlayer({ model, name: 'counter-model' }).chooseAction(state);
  assert.match(request, /give one concise public-facing reason based only on information everyone can see/);
  assert.match(request, /not merely an action caption/);
  assert.match(request, /Never reveal exact cards in your hand/);
  assert.match(request, /IF YOU ACCEPT: YOU give 1 wheat to player 1; YOU receive 1 brick from player 1/);
  assert.match(request, /Counteroffer \(parameterized\)/);
  assert.match(request, /counter 0\/1\/0\/0\/0 for 1\/0\/0\/0\/0/);
  assert.match(request, /That means you give 1 wheat and receive 1 brick/);
  assert.deepEqual(choice.action, { type: 'counterTrade', give: [0, 2, 0, 0, 0], receive: [1, 0, 0, 0, 0] });
});

test('live communication prompts retain Islanders-specific ownership and trade speech rules', async () => {
  const state = new IslandersState({ numPlayers: 2, rng: rng(), seatNames: ['Claude', 'the human player'] });
  let request = '';
  const model = new MockLanguageModelV3({
    doGenerate: async (options) => {
      request = JSON.stringify(options.prompt);
      return ({
        content: [{ type: 'text', text: JSON.stringify({
          thinking: 'setup', move: state.actionToString(state.legalActions()[0]),
          communication: { mode: 'silent', intent: 'none', text: '', privateReason: 'routine', respondsTo: '', addressedSeats: [] },
        }) }],
        finishReason: { unified: 'stop', raw: undefined },
        usage: { inputTokens: { total: 1 }, outputTokens: { total: 1 } }, warnings: [],
      }) as unknown as Awaited<ReturnType<MockLanguageModelV3['doGenerate']>>;
    },
  });
  const player = createIslandersModelPlayer({
    model,
    name: 'Claude',
    communication: { mode: () => 'ambient', guide: 'Ambient host policy.' },
  });
  await player.chooseAction(state);
  assert.match(request, /Name an affected opponent rather than saying a bare/);
  assert.match(request, /For a trade, plainly say what I give and what I receive/);
  assert.match(request, /communication must describe the exact canonical action/);
  assert.match(request, /facts as authoritative/);
  assert.match(request, /canonical move is only the text before any opening bracket/);
  assert.match(request, /Use .*I.* and .*my.* for your own position/);
});

...
export function detectIslandersMoments(
  before: IslandersState,
  action: IslandersAction,
  after: IslandersState,
  actorSeat: number,
  actionNumber: number,
  labels: readonly string[],
): GameMoment[] {
  const moments: GameMoment[] = [];
  const label = (seat: number): string => labels[seat] ?? `player ${seat + 1}`;
  const make = (moment: Omit<GameMoment, 'id' | 'game' | 'actorSeat' | 'actorLabel' | 'affectedLabels'>): void => {
    moments.push({
      id: `islanders-${actionNumber}-${moments.length + 1}`,
      game: 'islanders',
      actorSeat,
      actorLabel: label(actorSeat),
      affectedLabels: moment.affectedSeats.map(label),
      ...moment,
    });
  };

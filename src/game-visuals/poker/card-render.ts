...
export type CardFaceTextureProvider = (card: Card) => Texture;
...
export function drawCard(target: RenderTarget, vp: Mat4, M: Mat4, card: Card, back: Texture, bright = 1, faceTexture: CardFaceTextureProvider = pokerCardFaceTexture): void {
  const faceModel = mat4Multiply(M, FACE_OFFSET);
  rasterize(target, CARD_MESH, coverMaterial, {
    mvp: mat4Multiply(vp, faceModel),
    model: faceModel,
    tex: faceTexture(card),
    paper: WHITE,
    lightDir: LIGHT,
    ambient: 0.62,
    brightness: bright,
...
export function drawPeekCard(target: RenderTarget, vp: Mat4, pose: PeekPose, card: Card, back: Texture, bright = 1, faceTexture: CardFaceTextureProvider = pokerCardFaceTexture): void {
  const id = IDENTITY;
...
export function drawArchCard(target: RenderTarget, vp: Mat4, place: ArchPlace, card: Card, back: Texture, bright = 1, faceTexture: CardFaceTextureProvider = pokerCardFaceTexture): void {
  const id = IDENTITY;

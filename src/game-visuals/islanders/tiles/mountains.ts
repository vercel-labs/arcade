...
import { EDGE_Y, hexCorners, R_RIM, rimAndWall, terrainPerimeterSkirt } from '../base.ts';
...
export function mountainsTile(seed: number): Build {
  const m = build();
  ...
  terrainPerimeterSkirt(m, height, ROCK, M);
  rimAndWall(m, ROCK);
  return m;
}

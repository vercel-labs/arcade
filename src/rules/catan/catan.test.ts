...
const mutable = (state: CatanState): MutableCatan => state as unknown as MutableCatan;

function setHand(state: CatanState, player: number, values: Partial<Record<(typeof RESOURCES)[number], number>>): void {
...
test('each initial road must touch the settlement just placed', () => {
  ...
  const intendedRoad = s.legalActions().at(-1) as Extract<CatanAction, { type: 'initialRoad' }>;
  assert.deepEqual(
...
test('robber choices expose eligible victims and replay validates the stolen resource', () => {
  ...
  const action: CatanAction = { type: 'moveRobber', hex: destination, victim: 1 };
...
test('large discard spaces are bounded for enumeration but remain fully valid as a parameterized family', () => {
  ...
  const parameterized: CatanAction = {
    type: 'discard',
    resources: [
...
function mainPhaseWithDev(type: Exclude<DevCardType, 'victoryPoint'>): CatanState {
  const s = fresh();
  finishSetup(s);
  s.applyAction({ type: 'roll' }, { dice: [1, 1] });
  const internals = mutable(s);
...
test('special awards, hidden victory points, and model observations preserve public/private boundaries', () => {
  ...
  assert.match(opponentView, /P0: 9 public VP/);
  assert.doesNotMatch(opponentView, /P0.*victoryPoint/);
});
...
test('seeded random full games reach a winner without dead ends or resource drift', () => {
  for (let seed = 1; seed <= 5; seed++) {
    ...
    const s = new CatanState({ numPlayers: 4, rng: random });
    let actions = 0;
...
test('a transcript contains its initial world and replays without an external UI snapshot', () => {
  ...
  const replay = CatanState.replay(transcript, rng(999));

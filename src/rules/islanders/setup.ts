...
function officialHexSpiral(): number[] {
  ...
  for (let radius = 2; radius >= 1; radius--) {
    ...
    for (const [dq, dr] of COUNTERCLOCKWISE_RING_DIRS) {
      for (let step = 0; step < radius; step++) {
        const id = at.get(`${q},${r}`);
        if (id === undefined) throw new Error(`Missing Islanders hex at ${q},${r}`);
        order.push(id);
        q += dq;
        r += dr;
      }
  ...
  if (center === undefined) throw new Error('Missing center Islanders hex');
  order.push(center);
  return order;
}
...
function placeTokens(hexes: HexSetup[]): void {
  ...
  if (tokenIndex !== NUMBER_TOKENS.length) throw new Error(`Placed ${tokenIndex} Islanders number tokens`);
}
...
// heuristic AI value function (docs/islanders.md §3.7); pure over a BoardSetup.

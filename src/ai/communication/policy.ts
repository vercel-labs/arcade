...
export class CommunicationPolicy {
  ...
  decide(input: CommunicationPolicyInput): CommunicationDecision {
    ...
    const score = direct ? 1 : Math.max(0, Math.min(1, input.actionSalience + intentBonus + silenceBonus - repetitionPenalty - monologuePenalty));
    if (score < threshold) {
      ...
      return { communication, proposed: proposal, score, threshold, requiredResponse: input.requiredResponse, reason: 'below ambient threshold' };
    }
    return this.accept(input, score, threshold, direct ? 'direct response' : 'ambient threshold met');
  }

  private accept(input: CommunicationPolicyInput, score: number, threshold: number, reason: string): CommunicationDecision {
    ...
    return { communication: proposal, proposed: proposal, score, threshold, requiredResponse: input.requiredResponse, reason };
  }
}

// The Islanders match setup: an in-scene settings panel down the top-left of the board view
...
// "new match" button is live immediately. State lives on module-level instances so it
...
import { ISLANDERS_DEFAULT_AI_SEATS } from './islanders-defaults.ts';
...
export function setIslandersSetupChanged(fn: () => void): void {
  onChanged = fn;
}
const changed = (): void => {
  onChanged?.();
...
const sides: ModelSeatPicker[] = ISLANDERS_DEFAULT_AI_SEATS.map(({ creator, model }, i) => createModelSeatPicker({
...
export const communicationDropdown = new Dropdown({
  id: 'islanders-setup-communication',
...
export function islandersSeatColors(): PlayerColor[] {
...
export function mountIslandersSetup(ui: Screen): void {
...
export function islandersSetupSelection(): IslandersSeatSpec[] | null {
  if (!islandersSetupReady()) return null;
...
export function buildIslandersSetupPanel(): Node {
  ...
  return Box({ flexDirection: 'column', gap: 1, alignItems: 'start' }, [
    matchSetupHeading(),
    row('mode', Slot('islanders-setup-mode')),
    row('players', Slot('islanders-seats')),
    row('communication', Box({ gap: 2, alignItems: 'center' }, [
      Slot('islanders-setup-communication', { width: COMMUNICATION_W }),

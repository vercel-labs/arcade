...
test('Cover Flow keeps precise nudges and accelerates sustained horizontal swipes', () => {
  ...
  assert.equal(input.step({ wheel: 1, wheelAxis: 'horizontal' }, 1_180), 3);
  assert.equal(input.step({ wheel: 1, wheelAxis: 'horizontal' }, 1_240), 4);
  assert.equal(input.step({ wheel: 1, wheelAxis: 'horizontal' }, 1_300), 4);
...
test('vertical scrolling remains one cover per report and ends horizontal acceleration', () => {
  ...
  assert.equal(input.step({ wheel: 1, wheelAxis: 'horizontal' }, 1_000), 1);
  assert.equal(input.step({ wheel: 1, wheelAxis: 'horizontal' }, 1_030), 2);

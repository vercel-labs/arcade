...
import { Box, Button, Dropdown, type LayoutBox, type Node, type Screen, Slot, Text } from '../../../tui/index.ts';
...
// The full-screen HUD: a translucent control panel (top-left), a ☰ menu button (top-right,
// opens the in-game menu popup), the standard bar beneath. `bar` is buildBar('catan-tiles',
// …) from main; `onOpenMenu` opens the game menu.
export function buildCatanTileRoot(region: LayoutBox, bar: Node, onOpenMenu: () => void): Node {
  const panel = Box({ flexDirection: 'column', gap: 1, padding: [1, 2], background: [16, 18, 26, 0.9] }, [labeled('Tile', Slot('catan-terrain'))]);

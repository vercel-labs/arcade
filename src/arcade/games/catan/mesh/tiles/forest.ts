...
function forestParts(
  ...
  for (const p of pts) {
    ...
    if (!pineTarget) continue;
    const wind = sampleWind(time, origin.x + p.x, origin.z + p.z);
    const wave = Math.sin(time * 1.18 + treeSeed * 1.91 + p.x * 2.4 + p.z * 1.7);
    const response = 0.72 + 0.28 * wave;
    const flutter = Math.sin(time * 1.53 + treeSeed * 0.73) * 0.13 * wind.strength;
    const windX = wind.x - wind.z * flutter;
    const windZ = wind.z + wind.x * flutter;
    pine(pineTarget, p.x, p.z, hAt(p.x, p.z), scale, green, treeSeed, {
      windX,
      windZ,
...
export function forestTile(seed: number): Build {
  ...
  forestParts(seed, hAt, m, null, 0, { x: 0, z: 0 });
  return m;
}
...
export function animatedForestTile(seed: number, time: number, origin: WindOrigin): Build {
  ...
  const gseed = seed + 3.1;
  const hAt = (x: number, z: number): number => surfaceY(x, z, amp, gseed);
  forestParts(seed, hAt, null, m, time, origin);
  return m;
}

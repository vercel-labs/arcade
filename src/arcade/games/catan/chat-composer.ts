...
export function catanChatMentionCandidates(
  value: string,
  caret: number,
...
export function completeCatanChatMention(
  value: string,
  caret: number,
  target: ChatTarget,
...
export function parseCatanChatMentions(text: string, available: readonly ChatTarget[]): number[] {
  return available
    .slice()
...
function refreshSuggestions(): void {
  suggestions = catanChatMentionCandidates(input.value, input.caret, targets);
...
function completeSuggestion(target = suggestions[suggestionIndex]): boolean {
  ...
  const completed = completeCatanChatMention(input.value, input.caret, target);
...
const input = new Input({
  ...
  onEnter: (value) => {
    const addressedSeats = parseCatanChatMentions(value, targets);
  ...
  return Box({ position: 'relative', width: 36, height: rows, overflow: 'visible' }, [
    ...
    ...(suggestions.length
      ? [Box({
          ...
          id: `catan-chat-mention-${target.seat}`,

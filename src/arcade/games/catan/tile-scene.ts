...
export class TileScene {
  ...
  boardPortLabels(cols: number, rows: number): SailLabel[] {
    ...
    for (const harbor of this.harbors) {
      const c = mat4MulVec4(vp, { ...harbor.sailCenter, w: 1 });
      if (c.w <= 0) continue;
  ...
  renderScene(target: RenderTarget, t = 0): void {
    ...
    else if (this.modeName === 'pieces') rasterize(target, piecesMesh(this.pieceColor), catanPieceMaterial, { mvp: mat4Multiply(vp, MODEL), model: MODEL, lightDir: PIECE_LIGHT, ambient: PIECE_AMBIENT, wrap: PIECE_WRAP });
  ...
  private renderBoard(target: RenderTarget, vp: Mat4, eye: Vec3, t: number): void {
    ...
    rasterize(target, WATER_MESH, waterMaterial, waterUniforms);
    if (this.placing) {
      if (this.lastT < 0) this.lastT = t;
      this.placeClock += Math.max(0, t - this.lastT);
      this.lastT = t;
      if (this.placeClock > BOARD_BUILD_END) {
        this.placing = false;
    ...
    const coastProgress = this.placing
      ? Math.max(0, Math.min(1, (this.placeClock - COAST_GROW_START) / COAST_GROW_DUR))
      : 1;
    if (coastProgress > 0) {
      const coast = coastProgress >= 1 ? COAST_MESH : coastMesh(coastProgress);
      rasterize(target, coast, lambertMaterial, { mvp: mat4Multiply(vp, MODEL), model: MODEL, lightDir: LIGHT, ambient: 0.72, wrap: 1 });
    }
    ...
    if (coastProgress >= 1) {
      rasterize(target, swashMesh(t), waterMaterial, waterUniforms);
      rasterize(target, surfMesh(t), lambertMaterial, { mvp: mat4Multiply(vp, MODEL), model: MODEL, lightDir: LIGHT, ambient: 0.82, wrap: 1 });
    }
    if (this.revealing) {
    ...
    if (!this.placing && this.harborPiers) {
      rasterize(target, this.harborPiers, lambertMaterial, { mvp: mat4Multiply(vp, MODEL), model: MODEL, lightDir: LIGHT, ambient: AMBIENT, wrap: WRAP });
    }
    for (let i = 0; i < this.harbors.length; i++) {
      const harbor = this.harbors[i];
      const entry = this.placing ? harborEntryProgress(this.placeClock, i) : 1;
      if (entry <= 0) continue;
      const bridgeProgress = smooth(Math.max(0, Math.min(1, (entry - 0.62) / 0.38)));
      if (this.placing && bridgeProgress > 0) {
        const piers = harborPiersMesh([harbor.connector], bridgeProgress);
        rasterize(target, piers, lambertMaterial, { mvp: mat4Multiply(vp, MODEL), model: MODEL, lightDir: LIGHT, ambient: AMBIENT, wrap: WRAP });
      }
      const harborModel = this.placing
        ? harborEntryModel(harbor, entry, i, this.harborEntryDistances[i] ?? 0)
        : harbor.model;
      rasterize(target, portMesh(harbor.kind, this.boardSeed * 31 + i), lambertMaterial, {
        mvp: mat4Multiply(vp, harborModel),
        model: harborModel,
        lightDir: LIGHT,
        ambient: 0.62,
        wrap: 1,
  ...
  private renderOverlay(target: RenderTarget, vp: Mat4, t: number): void {
    ...
    rasterize(target, boardOverlayMesh(spec), catanPieceMaterial, { mvp: mat4Multiply(vp, MODEL), model: MODEL, lightDir: PIECE_LIGHT, ambient: PIECE_AMBIENT, wrap: PIECE_WRAP });
  }

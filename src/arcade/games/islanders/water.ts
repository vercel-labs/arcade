...
export const ISLANDERS_WATER_RADIUS_X = ISLANDERS_WATER_RADIUS;
export const ISLANDERS_WATER_RADIUS_Z = ISLANDERS_WATER_RADIUS;
export const ISLANDERS_WATER_Y = -0.19; // just below the tile walls (which bottom out at -0.16)
export const ISLANDERS_WATER_SUBDIVISIONS = 20;
...
export interface IslandersWaterMeshOptions {
...
export function islandersWaterMesh(options: IslandersWaterMeshOptions = {}): Mesh {
  const vertices: VertexIn[] = [];
  const byCoord = new Map<string, number>();
  const stepX = ISLANDERS_WATER_RADIUS_X / ISLANDERS_WATER_SUBDIVISIONS;
  const stepZ = ISLANDERS_WATER_RADIUS_Z / ISLANDERS_WATER_SUBDIVISIONS;
  ...
  for (let q = -ISLANDERS_WATER_SUBDIVISIONS; q <= ISLANDERS_WATER_SUBDIVISIONS; q++) {
    const rMin = Math.max(-ISLANDERS_WATER_SUBDIVISIONS, -q - ISLANDERS_WATER_SUBDIVISIONS);
    const rMax = Math.min(ISLANDERS_WATER_SUBDIVISIONS, -q + ISLANDERS_WATER_SUBDIVISIONS);
    for (let r = rMin; r <= rMax; r++) {
      const x = stepX * (Math.sqrt(3) / 2) * q;
      const z = stepZ * (r + q / 2);
      byCoord.set(key(q, r), vertices.length);
      vertices.push({
        position: { x, y: ISLANDERS_WATER_Y, z },
        normal: UP,
        uv: [0.5 + x / (ISLANDERS_WATER_RADIUS_X * 2), 0.5 + z / (ISLANDERS_WATER_RADIUS_Z * 2)],
        color: COLOR,
      });
    }
  }
  ...
  for (let q = -ISLANDERS_WATER_SUBDIVISIONS; q <= ISLANDERS_WATER_SUBDIVISIONS; q++) {
    for (let r = -ISLANDERS_WATER_SUBDIVISIONS; r <= ISLANDERS_WATER_SUBDIVISIONS; r++) {

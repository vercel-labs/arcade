// The Islanders tile test bed's HUD: a small control panel (top-left) whose dropdown switches
...
import { type DevCardType, type PlayerColor, type Resource, type Terrain } from '../../../rules/islanders/types.ts';
import { type BoardToken, type IslandersMode, type SailLabel } from './tile-scene.ts';
...
const CHIP_BG = ISLANDERS_NUMBER_TOKEN.background;
const CHIP_INK = ISLANDERS_NUMBER_TOKEN.ink;
const CHIP_RED = ISLANDERS_NUMBER_TOKEN.red;
const CHIP_GOLD = ISLANDERS_NUMBER_TOKEN.hot;
const CHIP_GOLD_INK = ISLANDERS_NUMBER_TOKEN.hotInk;
const CHIP_BLOCKED = ISLANDERS_NUMBER_TOKEN.blocked;
const CHIP_BLOCKED_INK = ISLANDERS_NUMBER_TOKEN.blockedInk;
...
type IslandersFlyingCard = Resource | DevCardType;

function isFlyingResource(card: IslandersFlyingCard): card is Resource {
  return (RESOURCE_ORDER as readonly string[]).includes(card);
}

function flyingCard(f: FlyingResource<IslandersFlyingCard>): Node {
  const { emoji, fill } = isFlyingResource(f.resource)
    ? islandersResourceFace(f.resource)
...
export function islandersFlyingCardNodes(
...
// dice. Keeping the node construction here guarantees the two Islanders covers render identically.
export function islandersProjectedBoardLabels(
...
export interface IslandersTileHandlers {
  onTerrain(t: Terrain): void;
  onReroll(): void;
  onToggleRobber(on: boolean): void;
  onMode(mode: IslandersMode): void;
  onRollDice(): void;
...
export function setIslandersTileHandlers(h: IslandersTileHandlers): void {
  H = h;
}

const modeDropdown = new Dropdown({ id: 'islanders-mode', items: MODE_LABELS, width: 18, index: 0, onSelect: (i) => H?.onMode(MODES[i]) });
const terrainDropdown = new Dropdown({ id: 'islanders-terrain', items: LABELS, width: 24, index: 0, onSelect: (i) => H?.onTerrain(TERRAINS[i]) });
const colorDropdown = new Dropdown({ id: 'islanders-color', items: COLOR_LABELS, width: 14, index: 0, onSelect: (i) => H?.onColor(COLORS[i]) });
const portDropdown = new Dropdown({ id: 'islanders-port', items: PORT_LABELS, width: 16, index: 0, onSelect: (i) => H?.onPort(PORT_KINDS[i]) });

export function mountIslandersTileHud(ui: Screen): void {
  ui.mount(modeDropdown);
  ui.mount(terrainDropdown);
  ui.mount(colorDropdown);
  ui.mount(portDropdown);
  mountIslandersCardsHud(ui); // the card overlay's scrollable action history
}

// The terrain the dropdown currently shows (its committed selection).
export function islandersTileTerrain(): Terrain {
...
export function setIslandersTileMode(mode: IslandersMode): void {
...
export function buildIslandersPieceModal(o: PieceModalOpts): Node {
...
export function buildIslandersTileRoot(region: LayoutBox, onOpenMenu: () => void, tokens: BoardToken[] = [], mode: IslandersMode = 'tile', sails: SailLabel[] = [], flights: FlyingResource<IslandersFlyingCard>[] = [], movingRobber = false, cardsView?: IslandersCardsView): Node {
  ...
  const railOpen = mode === 'boardCards' && islandersSidebarOpen();
  ...
  const chrome = Box({ width: region.w, height: region.h }, [
    ...
    ...islandersFlyingCardNodes(flights),
    ...(movingRobber
      ? [hudTopCenter(
          ...
          ]), region.w, { railWidth: railOpen ? ISLANDERS_RAIL_W : 0 })]
      : []),
    ...
    hudTopRight([
      Button({ id: 'islanders-menu-button', label: MENU_BUTTON_LABEL, onClick: onOpenMenu, style: UI_CHROME_PILL }),
      ...(mode === 'boardCards' && !railOpen ? [Button({ id: 'islanders-sidebar-open', label: 'sidebar', onClick: toggleSidebar, style: UI_CHROME_PILL })] : []),
    ], { railWidth: railOpen ? ISLANDERS_RAIL_W : 0 }),
  ...
  return Box({ width: region.w, height: region.h }, [
    ...islandersProjectedBoardLabels(tokens, sails), // above the board, below foreground 3D
    chrome,
  ]);
}

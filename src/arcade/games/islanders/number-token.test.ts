...
import { TOKEN_DOTS } from '../../../rules/islanders/types.ts';
import { IslandersState } from '../../../rules/islanders/islanders.ts';
import { layout, type Node, Screen } from '../../../tui/index.ts';
import { buildIslandersTileRoot, islandersProjectedBoardLabels } from './tile-hud.ts';
...
test('projected board labels preserve every production token for the real-game HUD', () => {
  ...
  assert.equal(tokens.length, 18);
  assert.equal(islandersProjectedBoardLabels(tokens).length, 18);
});
...
test('an active game preserves the preview board and reveals numbers before the first placement', async () => {
  ...
  assert.ok(preview);
  const state = new IslandersState({ numPlayers: 4, board: preview });
...
test('the token overlay renders compact unspaced bullet pips only when requested', () => {
  ...
  const detailed = texts(buildIslandersTileRoot({ x: 0, y: 0, w: 80, h: 40 }, () => {}, [token], 'board'));
  assert.ok(detailed.includes('6'));
  assert.ok(detailed.includes('•••••'));

  const distant = texts(buildIslandersTileRoot({ x: 0, y: 0, w: 80, h: 40 }, () => {}, [{ ...token, showPips: false }], 'board'));
...
test('number-token overlays do not intercept board clicks', () => {
  ...
  screen.setRoot(buildIslandersTileRoot(region, () => {}, [token], 'board', [], [], true), region);
...
test('detailed token rows straddle the projected center while number-only tokens sit on it', () => {
  const region = { x: 0, y: 0, w: 80, h: 40 };
  const token = { col: 20, row: 10, num: 6, pips: 5, showPips: true, red: true, hot: false, blocked: false };
  const detailed = buildIslandersTileRoot(region, () => {}, [token], 'board');
  layout(detailed, region);
  assert.equal(findText(detailed, '6')?.layout?.y, 9);
  assert.equal(findText(detailed, '•••••')?.layout?.y, 10);

  const distant = buildIslandersTileRoot(region, () => {}, [{ ...token, showPips: false }], 'board');
...
test('all parity combinations use regular bullets centered with left-biased ties', () => {
  const region = { x: 0, y: 0, w: 80, h: 40 };
  const threePips = buildIslandersTileRoot(region, () => {}, [
  ...
  const fivePips = buildIslandersTileRoot(region, () => {}, [
  ...
  const onePip = buildIslandersTileRoot(region, () => {}, [
...
test('the Islanders test HUD announces the robber gate and replaces the roll action', () => {
  const root = buildIslandersTileRoot({ x: 0, y: 0, w: 100, h: 40 }, () => {}, [], 'board', [], [], true);

// The Islanders tile test bed: two modes, switched from the HUD.
...
export type IslandersMode = 'tile' | 'board' | 'boardCards' | 'pieces' | 'port';

function isBoardMode(mode: IslandersMode): boolean {
...
function isInsideBoardWater(x: number, z: number): boolean {
  const ax = Math.abs(x);
  const az = Math.abs(z);
  const xLimit = ISLANDERS_WATER_RADIUS_X * Math.sqrt(3) / 2;
  const zLimit = ISLANDERS_WATER_RADIUS_Z - (ax * ISLANDERS_WATER_RADIUS_Z) / (Math.sqrt(3) * ISLANDERS_WATER_RADIUS_X);
...
export class TileScene {
  ...
  private modeName: IslandersMode = 'tile';
  ...
  private readonly piecePool = new ObjectPool(() => new MeshObject(
    EMPTY_MESH,
    new WorldMaterialInstance<IslandersPieceUniforms>(islandersPieceMaterial, {
  ...
  // A road is placeable for the active color per the Islanders connectivity rules: it must extend a

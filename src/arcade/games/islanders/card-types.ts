...
import type { MaritimePortTradeRates, MaritimeTradeRates } from '../../../rules/islanders/maritime-trade.ts';
...
export interface IslandersCardsPlayerView {
  seat?: number;
  name: string;
  color: PlayerColor;
  publicVp: number;
...
export interface IslandersActionHistoryView {
  actor: string;
  color: PlayerColor;
  message: string;
  resources?: Resource[];
...
export interface IslandersDevelopmentPlayView {
  type: Exclude<DevCardType, 'victoryPoint'>;
  remaining: number;
  resources: Resource[];
}

export interface IslandersCardsView {
  // The shared HUD renders both the real game and the experimental Islanders-Test cover. Keep the
  // source explicit so test-only controls and dummy data cannot leak into a live game view.
  source: 'live' | 'workbench';
  localPlayer: IslandersCardsPlayerView;
  ...
  history: IslandersActionHistoryView[];
  ...
  // One muted tooltip line per held card that cannot be played right now.
  developmentCardHolds?: Partial<Record<DevCardType, string>>;
  ...
  developmentPlay?: IslandersDevelopmentPlayView;
}

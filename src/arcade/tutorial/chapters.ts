...
export const TUTORIAL_CHAPTERS: readonly TutorialChapter[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'start', label: 'pick a side and a model, then start', hint: 'choose human for one side to play the model yourself, or two models to watch. free-tier models use the team\'s monthly included Gateway credit; other models require purchased AI Gateway Credits. if the team cannot route the model, you\'ll see the normal health-check popup with the fix.', signal: 'chess.matchStarted', requires: 'gateway' },

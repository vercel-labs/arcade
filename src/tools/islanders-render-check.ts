// Visual regression check for the Islanders scene. Renders a fixed set of views and fingerprints
// each frame, so a refactor can be *proven* not to change what's drawn rather than eyeballed.
//
//   pnpm islanders:check capture    # record the current renders as the baseline
//   pnpm islanders:check            # re-render and report which views changed (exit 1 if any)
//
...
import { type PortKind } from '../arcade/games/islanders/mesh/index.ts';
import { buildIslandersTileRoot, mountIslandersTileHud } from '../arcade/games/islanders/tile-hud.ts';
import { ISLANDERS_LOCAL_COLOR, islandersHandLandingCell, islandersSidebarOpen, toggleIslandersSidebar } from '../arcade/games/islanders/card-hud.ts';
import { ResourceFlights } from '../arcade/games/islanders/scene/resource-flight.ts';
...
import { type PlayerColor, type Terrain, TERRAINS } from '../rules/islanders/types.ts';

const MANIFEST = '.snapshots/islanders-render.manifest';
const SS = 4; // supersample factor, matching `pnpm snapshot islanders`
...
// Paint `root` over a scene render, the way the app composites Islanders (no hybrid shading, so the
...
function tileComposite(setup: (s: TileScene) => void, opts: { time?: number; rail?: boolean; flights?: (s: TileScene, w: number, h: number) => ResourceFlights } = {}): (cols: number, rows: number) => Surface {
  return (cols, rows) => {
    ...
    setup(scene);
    const wasOpen = islandersSidebarOpen();
    if (!!opts.rail !== wasOpen) toggleIslandersSidebar();
    try {
      ...
      return composite(
        ...
        (screen) => {
          ...
          screen.setRoot(
            buildIslandersTileRoot(region, () => {}, scene.boardTokens(cols, rows), scene.currentMode(), singlePort ? [singlePort] : scene.boardPortLabels(cols, rows), flights?.active() ?? [], scene.isMovingRobber()),
            region,
          );
        },
      );
    } finally {
      if (islandersSidebarOpen() !== wasOpen) toggleIslandersSidebar();
    }
  };
}
...
function rollFlights(roll: number, at: number): (s: TileScene, w: number, h: number) => ResourceFlights {
  return (scene, w, h) => {
    ...
    let thrown = 0;
    for (const source of scene.rollSources(ISLANDERS_LOCAL_COLOR, roll, w, h)) {
      flights.spawn(source.resource, source.count, source, islandersHandLandingCell({ x: 0, y: 0, w, h }, source.resource), thrown);
      thrown += source.count;
    }
...
function gameComposite(opts: { plies?: number; setup?: boolean } = {}): (cols: number, rows: number) => Surface {
  return (cols, rows) => {
    ...
    const driver = new IslandersDriver({ scene: gameScene, syncLive: () => {} });
    if (!opts.setup) {
      const colors: PlayerColor[] = ['red', 'blue', 'purple', 'orange'];
      const specs: IslandersSeatSpec[] = colors.map((color, i) => (i === 0 ? { kind: 'human', color } : { kind: 'ai', color, model: 'openai/gpt-5.4-nano' }));
      const state = driver.start(specs, { autoRun: false, rng: mulberry32(0xca7a4) });
      gameScene.setResourceFlightLayout({ x: 0, y: 0, w: cols, h: rows }, colors.length, islandersSidebarOpen());
...
if (mode === 'capture') {
  ...
  try {
    ...
    console.error(`no baseline at ${MANIFEST} — run \`pnpm islanders:check capture\` on the known-good tree first`);
    process.exit(2);
  }
  const now = render();
  ...
  console.error(`usage: pnpm islanders:check [capture|compare]`);
  process.exit(2);
}

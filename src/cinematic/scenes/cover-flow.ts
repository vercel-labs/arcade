...
export interface CoverFlowItem { id: string; title: string; enabled: boolean }
...
export function coverFlowCinematicState(progress: number, itemCount: number): CoverFlowCinematicState {
  ...
  const pos = p < 0.63 ? lerp(0, Math.max(0, itemCount), smoothstep(p / 0.63)) : Math.max(0, itemCount);
...
export class CoverFlowRenderer {
  ...
  renderScene(target: RenderTarget, pos: number, hoverIndex = -1): void {
    ...
    for (const slot of order) {
      ...
      drawCover(target, viewProjection, model, texture, COVER_BRIGHT, false, slot === hoverIndex);
    }
  }
  ...
  renderCinematic(target: RenderTarget, pos: number, launchIndex: number, launchProgress: number): void {
    ...
    for (const slot of order) {
      ...
      const texture = backFacing ? titleTexture(item.title) : art;
  ...
  renderLaunchProgress(target: RenderTarget, index: number, progress: number): void {
    ...
    rasterize(target, CARD_MESH, coverMaterial, {
      mvp, model, tex: Math.abs(angle) > Math.PI / 2 ? titleTexture(item.title) : art,
...
function titleTexture(title: string): Texture {
  ...
  columns.pop();
  const textWidth = columns.length * LAUNCH_TITLE_PX;
  const textHeight = 8 * LAUNCH_TITLE_PX;
  const side = Math.max(textWidth, textHeight) + 12 * LAUNCH_TITLE_PX;
  const ox = Math.floor((side - textWidth) / 2), oy = Math.floor((side - textHeight) / 2);
  ...
  for (let cx = 0; cx < columns.length; cx++) for (let row = 0; row < 8; row++) {
    ...
    for (let py = 0; py < LAUNCH_TITLE_PX; py++) for (let px = 0; px < LAUNCH_TITLE_PX; px++) {
      ...
      const y = oy + row * LAUNCH_TITLE_PX + py;
      const i = (y * side + x) * 4;

// The prism streaming handler — ONE implementation shared by the Vercel function
// ([api/index.ts](../../api/index.ts)) and the standalone local server
// (src/tools/serve-prism.ts), so local testing is byte-identical to production.
// Serves the splash → rotating-prism animation as ANSI so `curl -sN <host>` plays
// it in the terminal (the parrot.live pattern: the server streams escape codes, the
// terminal renders them). Output-only and looped, but FINITE: serverless functions
// have a max execution time, so the loop stops a few seconds before the cap and ends
// cleanly (re-run curl to replay). It's a plain Node (req, res) handler with no
// Vercel-specific APIs, so createServer(streamPrism) runs the same thing locally.
//
// Clients:
...
export async function streamPrism(req: IncomingMessage, res: ServerResponse): Promise<void> {

...
opaque 3D layers also have an optional local WebGPU/Dawn backend with automatic CPU fallback. Run with
...
- `pnpm smoke:gpu [cols] [rows]` — validate GPU bases and CPU overlays across every game
